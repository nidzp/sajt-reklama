...
console.log(
  `🚀 Starting load test: ${NUM_REQUESTS} concurrent requests to ${BASE_URL}/api/contact\n`
);
...
const sampleMessages = [
  {
    ...
    message:
      "I'm interested in hiring you for a web development project. Can we discuss pricing?",
  },
  {
    name: "Jane Smith",
    email: "jane@example.com",
    message:
      "I'd like to collaborate on a mobile app. Are you available for partnership?",
  },
  {
    name: "Bob Johnson",
    email: "bob@example.com",
    message:
      "Quick question about your portfolio - what technologies do you use?",
  },
  {
    name: "Alice Williams",
  ...
  },
  {
    ...
    message:
      "Your work looks amazing! I'd love to work together on my next project.",
  },
];
...
function sendRequest(index) {
  return new Promise((resolve, reject) => {
      ...
      res.on("end", () => {
        ...
        try {
          ...
          if (res.statusCode === 200) {
            console.log(
              `✅ Request ${index + 1}: SUCCESS (${duration}ms) - Intent: ${
                parsed.analysis?.intent
              }, Urgency: ${parsed.analysis?.urgency}`
            );
            resolve({ success: true, duration, data: parsed });
          } else {
            console.log(
              `❌ Request ${index + 1}: FAILED (${duration}ms) - Status: ${
                res.statusCode
              }, Error: ${parsed.error}`
            );
          ...
          console.log(
            `⚠️  Request ${index + 1}: PARSE ERROR (${duration}ms) - ${
              error.message
            }`
          );
    ...
    req.on("error", (error) => {
      const duration = Date.now() - startTime;
      console.log(
        `❌ Request ${index + 1}: NETWORK ERROR (${duration}ms) - ${
          error.message
        }`
      );
      resolve({ success: false, duration, error: error.message });
...
async function runLoadTest() {
  ...
  console.log(
    `Failed:            ${failed} (${((failed / NUM_REQUESTS) * 100).toFixed(
      1
    )}%)`
  );
  ...
  console.log(
    `ℹ️  Note: With semaphore limit of 3, expect:\n` +
